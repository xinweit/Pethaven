...
router.post("/", authorization, async (req, res) => {
    try {
        ...
        var query = "INSERT INTO advertisements(pet_category, start_date, end_date, daily_price, email) VALUES($1, $2, $3, $4, $5);";
        const newAdvertisement = await pool.query(query, [pet_category, start_date, end_date, daily_price, req.user]);
...
router.delete("/", authorization, async (req, res) => {
    try {
        ...
        var query = "DELETE FROM advertisements WHERE pet_category = $1 AND start_date = $2 AND end_date = $3 AND email = $4";
        const pew = await pool.query(query, [pet_category, start_date, end_date, req.user]);
        res.json(pew.rows);
    } catch (error) {
        console.error(error.message);
        res.status(500).json("Server Error");
