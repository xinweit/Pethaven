...
router.put("/", authorization, async (req, res) => {
	try {
		const { name } = req.body;
		const updateName = await pool.query("UPDATE pet_owners SET name = $1 WHERE email = $2", [
			name,
			req.user,
		]);
...
router.delete("/", authorization, async (req, res) => {
	try {
		...
		const deleteAccount = await pool.query("DELETE FROM pet_owners WHERE email = $1", [email]);
